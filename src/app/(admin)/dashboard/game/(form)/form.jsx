...
import {
  createGame,
  deleteFormAction,
} from "@/app/(admin)/dashboard/game/(form)/actions";
...
function SubmitButton() {
  ...
  return (
    <button
      ...
      className="w-full text-white bg-accent font-medium rounded-lg text-sm px-5 py-2.5 text-center"
    >
      {pending ? "Saving..." : "Save"}
    </button>
  );
      ...
      <form className="flex flex-col lg:flex-row gap-8" action={formAction}>
        ...
        <div className="w-full">
          <div>
            <label
              htmlFor="title"
              className="block mb-2 text-xs text-accent uppercase"
            >
              Title
            </label>
            <input
            ...
            >
              {categories?.map((category) => (
                <option key={category.id} value={category.id}>
                  {category?.id === game?.categories[0].id
